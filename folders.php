...
<style>
  ...
  *{box-sizing:border-box;margin:0}
  ...
  header{
    ...
    margin-left:var(--sideW); transition:margin-left .25s ease;
    position: sticky; top: 0; z-index: 50; width: calc(100% - var(--sideW));
  }
  ...
  main{ 
    ...
    padding: 20px var(--contentPad) 50px; 
  ...
  @media(min-width: 769px) {
      #themeToggle {
          width: 100px; /* Bigger than others (85px) */
          height: 100px;
      }
  }
  ...
  /* Enhanced Account Menu - Force Light BG */
  .account-menu{ 
  ...
  @media (max-width: 768px) {
    ...
    header { margin-left: 0; width: 100%; padding: 0 16px; }
    .mobile-toggle { display: block; }
    main { margin-left: 0; padding: 90px 16px 40px; }
    .folders-container { padding: 0 10px 60px !important; }
  }
  ...
  /* =========================================================
     2. FOLDERS SPECIFIC STYLES 
     ========================================================= */
  
  /* Hero */
  .folders-hero{ text-align:center; padding: 34px 16px 28px; }
  .folders-hero h2{ font-weight: 900; font-size: clamp(22px, 3vw, 40px); color: #0b2f42; margin: 0 0 6px; }
  .folders-hero p{ margin: 0 0 18px; color: #2d5160; opacity: .9; }
  
  /* FOLDERS DARK MODE TEXT */
  body.dark .folders-hero h2{ color:#e9f8ff; } 
  body.dark .folders-hero p{ color:#b8dce8; }

  /* Search & Buttons */
  .fh-search{ margin: 0 auto 20px; max-width: 560px; }
  .fh-search input{ width: 70%; height: 40px; border-radius: 999px; border: 2px solid #69bfd3; padding: 0 18px; outline: none; background: #fff; }
  .fh-actions{ display:flex; justify-content:center; gap:28px; }
  
  /* Enhanced Buttons */
  .btn{ 
    display:inline-flex; align-items:center; justify-content:center; 
    padding: 16px 34px; border-radius:999px; font-weight:800; cursor:pointer; 
    border:2px solid; transition: .2s; 
    box-shadow: 0 4px 15px rgba(2, 48, 71, 0.1);
  }
  .btn:hover{ transform: translateY(-3px); box-shadow: 0 8px 20px rgba(2, 48, 71, 0.2); }

  .btn.primary{ background:#0b2f42; color:#fff; border-color:#0b2f42; }
  .btn.outline{ background:#fff; color:#0b2f42; border-color:#69bfd3; }
  
  body.dark .btn.outline { background:transparent; color:#e4f7fb; border-color:#e4f7fb; }
  body.dark .btn.primary { background:#e4f7fb; color:#023047; border-color:#e4f7fb; }
  
  /* Quiz Lists */
  .folders-container{ padding: 0 40px 60px; max-width: 1200px; margin: 0 auto; }
  .section-title{ font-size: 20px; font-weight: 800; margin: 40px 0 10px 6px; text-transform: uppercase; }
  body.dark .section-title{ color:#e9f8ff; }

  .mc-list{ list-style: none; margin: 0 0 20px 0; display: flex; flex-wrap: wrap; gap: 24px; padding: 0; }
  .mc-item{ display: flex; flex-direction: column; align-items: center; gap: 8px; position: relative; }
  ...
  .mc-bubble:hover { transform: scale(1.05) translateY(-5px); box-shadow: 0 10px 25px rgba(0,0,0,0.1); }
  
  .bubble-0 { background: #ff78b5; } .bubble-1 { background: #dfb7ff; } 
  .bubble-2 { background: #ff6b6b; } .bubble-3 { background: #ffd56b; } 
  .bubble-4 { background: #78d46f; } .bubble-5 { background: #57b0ff; }

  .mc-actions{ position: absolute; inset: 0; display: flex; flex-direction: column; gap: 8px; justify-content: center; align-items: center; opacity: 0; transition: opacity .2s; background: rgba(0,0,0,0.1); border-radius: 50%; pointer-events: none; }
  .mc-item.open .mc-actions{ opacity: 1; pointer-events: auto; }

  .mc-btn{ padding: 8px 24px; border-radius: 999px; font-weight: 800; font-size: 12px; border: none; cursor: pointer; width: 80px; }
  .mc-btn.light{ background: #fff; color: #0b2f42; }
  .mc-btn.dark{ background: #0b2f42; color: #fff; }

  .mc-info h4{ font-size: 14px; font-weight: 900; color: #000; margin: 0; text-align: center; }
  .mc-info p{ font-size: 11px; color: #5a6c78; margin: 0; text-align: center; }
  
  body.dark .mc-info h4 { color: #e9f8ff; } 
  body.dark .mc-info p { color: #b8dce8; }
  
  .empty-text{ font-size: 13px; color: #5a6c78; margin-left: 6px; font-style: italic; }
  .no-results { display: none; font-size: 14px; color: #5a6c78; margin-top: 10px; font-style: italic; }

  /* =========================================================
     3. PROFILE VIEW STYLES
     ========================================================= */
  .profile-wrap{ max-width: 600px; padding: 30px 30px 52px; border-radius: 40px; background: #e4f7fb; box-shadow: 0 6px 24px rgba(0,0,0,.06); margin: 60px auto; }
  ...
  .drawer-backdrop{ position:absolute; inset:0; background:rgba(0,0,0,.45); backdrop-filter: blur(2px); opacity:0; transition:opacity .2s ease; }
  ...
  .dq-label{ font-size:.9rem; font-weight:800; color:#023047; margin:0 0 8px 4px; display:block; }
  .dq-input{ width:100%; height:46px; border-radius:14px; border:1.8px solid #218ca6; background:#fff; padding:0 14px; font-size:14px; color:#0b2330; outline:none; }
  .dq-select select{ appearance:none; width:100%; height:46px; border-radius:14px; border:1.8px solid #218ca6; background:#fff; padding:0 42px 0 14px; font-size:14px; color:#0b2330; outline:none; }
  ...
  /* Join Modal Styles */
  .modal { position: fixed; inset: 0; background: rgba(0,0,0,.55); display: none; justify-content: center; align-items: center; z-index: 3100; backdrop-filter: blur(3px); }
  .modal.show { display: flex; }
  .modal-content { background: #e4f7fb; border-radius: 22px; padding: 40px 38px 48px; width: 420px; box-shadow: 0 8px 26px rgba(0,0,0,.25); position: relative; text-align: left; animation: modalPop .18s ease-out; }
  ...
  .modal-content h3 { margin-bottom: 18px; font-weight: 800; color: #023047; font-size: 1.5rem; }
  .input-group { position: relative; margin-bottom: 16px; }
  .input-group input { width: 100%; height: 46px; border-radius: 12px; border: 1.8px solid #218ca6; background: #e4f7fb; padding: 0 16px; font-size: 14px; color: #300b0b; }
  .input-group input:focus { outline: none; box-shadow: 0 0 0 3px rgba(33,140,166,.5); }
  .input-group label{ position:absolute; left:16px; top:50%; transform:translateY(-50%); font-size:.95rem; font-weight:400; color:#023047; pointer-events:none; transition: .2s; }
  .input-group input:focus + label, .input-group input:not(:placeholder-shown) + label{ top:-10px; left:18px; transform:none; font-size:.85rem; font-weight:600; background:#e4f7fb; padding:0 6px; }

  /* Dark Mode specific overrides for Modal/Drawer - KEEP LIGHT */
  body.dark .drawer-panel, body.dark .modal-content { background:#e4f7fb; border-color:#218ca6; color:#000; }
  body.dark .dq-group { background:#e4f7fb; border-color:#69bfd3; }
  body.dark .dq-label, body.dark .modal-content h3 { color:#023047; }
  body.dark .dq-input, body.dark .dq-select select, body.dark .input-group input { background:#fff; color:#0b2330; border-color:#218ca6; }

  /* Toast & View Management */
  .view{ display:none; }
...
<body>
  ...
  <nav class="sidebar" aria-label="Sidebar">
    ...
    <a href="#" class="nav-item active" onclick="showView('folders'); return false;">
  ...
  <main>
    <section id="view-folders" class="view active">
        <section class="folders-hero">
        <h2>Your ThinkUp Space</h2>
        <p>Store your ideas, save your quizzes, and share them when you're ready.</p>
        ...
        <div class="fh-actions">
            <button class="btn primary" id="createQuizBtn">+ Create Quiz</button>
            <button class="btn outline" id="joinQuizBtn">+ Join Quiz</button>
        </div>
        </section>

        <div class="folders-container">
        <p id="searchNoResult" class="no-results">No quizzes found matching your search.</p>

        <h4 class="section-title">My Creations</h4>
        <?php if (empty($finished_quizzes)): ?>
            <p class="empty-text">No finished creations yet.</p>
        <?php else: ?>
            <ul class="mc-list" id="finishedList">
                <?php foreach ($finished_quizzes as $index => $quiz): $color_class = 'bubble-' . ($index % 6); ?>
                    <li class="mc-item" onclick="this.classList.toggle('open')">
                    <div class="mc-bubble <?php echo $color_class; ?>">
                        <div class="mc-actions">
                        <button class="mc-btn light" onclick="event.stopPropagation(); window.location.href='quiz-editor.php?id=<?php echo $quiz['id']; ?>'">Edit</button>
                        <button class="mc-btn dark" onclick="event.stopPropagation(); window.location.href='quiz-taking.php?code=<?php echo htmlspecialchars($quiz['quiz_code']); ?>'">Try It</button>
                        </div>
                        <span style="font-size:64px; font-weight:900; color:#0b2330;">
                            <?php echo strtoupper(substr($quiz['course_name'], 0, 1)); ?>
                        </span>
                    </div>
                    <div class="mc-info">
                        <h4><?php echo htmlspecialchars($quiz['course_name']); ?></h4>
        ...
        <?php if (empty($saved_quizzes)): ?>
            <p class="empty-text">No saved quizzes yet.</p>
        <?php else: ?>
            <ul class="mc-list" id="savedList">
                <?php foreach ($saved_quizzes as $index => $quiz): $color_class = 'bubble-' . ($index % 6); ?>
                    <li class="mc-item" onclick="this.classList.toggle('open')">
                    <div class="mc-bubble <?php echo $color_class; ?>">
                        <div class="mc-actions">
                        <button class="mc-btn light" onclick="event.stopPropagation(); window.location.href='quiz-editor.php?id=<?php echo $quiz['id']; ?>'">Edit</button>
        ...
        <?php else: ?>
            <ul class="mc-list" id="draftList">
                <?php foreach ($draft_quizzes as $index => $quiz): $color_class = 'bubble-' . ($index % 6); ?>
                    <li class="mc-item" onclick="this.classList.toggle('open')">
                    <div class="mc-bubble <?php echo $color_class; ?>">
                        <div class="mc-actions">
                        <button class="mc-btn light" onclick="event.stopPropagation(); window.location.href='quiz-editor.php?id=<?php echo $quiz['id']; ?>'">Edit</button>
      ...
      <div class="profile-wrap pi-card">
        ...
        <div class="pi-header">
          ...
          <div class="pi-name-group">
            <div class="pi-field">
              <input id="pi_first" class="profile-input" placeholder="First Name" value="<?php echo htmlspecialchars($user_name); ?>">
            </div>
            <div class="pi-field">
              <input id="pi_last" class="profile-input" placeholder="Last Name">
            </div>
          </div>
        </div>
  ...
  <div id="joinQuizModal" class="modal" aria-hidden="true">
    <div class="modal-content">
      <button class="modal-close" id="closeJoinModal">×</button>
      <h3>Join quiz</h3>
      <div class="input-group">
        <input id="join_code" type="text" placeholder=" " maxlength="6" inputmode="numeric" />
        <label for="join_code">6-Digit Code</label>
      </div>
      <div style="display:flex; justify-content:flex-end; margin-top:20px;">
        <button id="confirmJoin" class="dq-btn">Join</button>
      </div>
    </div>
  </div>
...
<script>
  ...
  // This moves the black indicator to the active tab immediately on load
  ...
  // UPDATED: Using the exact same Sun vector as home.php from previous context
  ...
  // --- Folders Page Logic ---
  // (Note: DOMContentLoaded listener merged above for active indicator, keeping logic clean)
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const lists = [document.getElementById('finishedList'), document.getElementById('savedList'), document.getElementById('draftList')];
    ...
    if (searchInput) {
      searchInput.addEventListener('input', (e) => {
        ...
        lists.forEach(list => {
          if(!list) return;
          list.querySelectorAll('.mc-item').forEach(item => {
             const txt = item.innerText.toLowerCase();
             if(txt.includes(val)){ item.style.display=''; visibleCount++; }
             else { item.style.display='none'; }
          });
        });
        document.getElementById('searchNoResult').style.display = (visibleCount===0 && val.length > 0) ? 'block' : 'none';
      });
    }

    const drawer = document.getElementById('quizDrawer');
    const dqSave = document.getElementById('dqSave');
    document.getElementById('createQuizBtn')?.addEventListener('click', ()=>drawer.classList.add('show'));
    const closeEls = [document.getElementById('dqClose'), document.getElementById('dqCancel'), document.querySelector('[data-close="drawer"]')];
    closeEls.forEach(el => el?.addEventListener('click', ()=>drawer.classList.remove('show')));
    ...
    dqSave?.addEventListener('click', () => {
      ...
      if (!name || !course) { alert('Please fill in Name and Course.'); return; }
    ...
    const jm = document.getElementById('joinQuizModal');
    const submitJoin = document.getElementById('confirmJoin');
    document.getElementById('joinQuizBtn')?.addEventListener('click', ()=>jm.classList.add('show'));
    document.getElementById('closeJoinModal')?.addEventListener('click', ()=>jm.classList.remove('show'));
  ...
  const saveAvatar = document.getElementById('saveAvatarBtn');
  let tempAvatar = null;
